...
def build_sms(current, alerts, top_ticker, top_stock):
    ...
    # Alerts — exclude top ticker (already shown), strip emojis, keep very short
    high = [a for a in alerts
            if any(x in a for x in ['LEAD','IMPR','WEAK','LAG','score'])
            and top_ticker not in a][:2]
    line3 = ""
    if high:
        clean = []
        for a in high:
